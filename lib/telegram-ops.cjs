...
function buildLogFingerprint({ level, source, scope, user, message, payload, chatId }) {
...
function buildHeaderLines({ level, source, scope, user, timestamp }) {
  const lines = [
    `<b>[${htmlEscape(level)}]</b>`,
    `<b>status:</b> ${htmlEscape(level)}`,
...
function buildLogChunks({ level, source, scope, userContext, message, payload, timestamp }) {
  const user = normalizeUserContext(userContext);
  const label = level === "ERROR" ? "payload" : level === "SUCCESS" ? "payload" : "payload";
  const summaryLabel = level === "ERROR" ? "reason" : level === "SUCCESS" ? "result" : "details";
  const header = buildHeaderLines({ level, source, scope, user, timestamp }).join("\n");
