...
let activeType = 'all', activeDate = 'all', draft = { receipt: null, rawFile: null }, selId = null, selIcon = 'circle';
...
document.addEventListener('DOMContentLoaded', async () => {
    lucide.createIcons();
    isBiometricAvailable = window.PublicKeyCredential && await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable().catch(() => false);
    ...
    if (card) {
        ...
        card.addEventListener('touchend', (e) => { handleDashboardSwipe(startX, e.changedTouches[0].screenX); }, { passive: true });
        card.addEventListener('mousedown', (e) => { startX = e.clientX; });
        card.addEventListener('mouseup', (e) => { handleDashboardSwipe(startX, e.clientX); });
    }
});

function handleDashboardSwipe(startX, endX) {
    const diff = startX - endX;
    if (diff > 50 && dashboardCurrency === 'UZS') setDashboardCurrency('USD');
    if (diff < -50 && dashboardCurrency === 'USD') setDashboardCurrency('UZS');
}
...
async function fetchInitialData() {
    ...
    if (userData?.exchange_rate) {
        exchangeRate = Number(userData.exchange_rate);
...
async function initDefaultCategories() {
    ...
    const all = defaults.map(c => ({ ...c, user_id: currentUserId }));
    const { error } = await supabase.from('categories').insert(all);
    if (!error) {
        allCats.income = defaults.filter(c => c.type === 'income');
        allCats.expense = defaults.filter(c => c.type === 'expense');
    }
}
...
function showPinScreen(step) {
    ...
    if (step === 'unlock') {
        titleEl.innerText = "PIN Kod";
        subtitleEl.innerText = "Kirish uchun";
        ...
        titleEl.innerText = "Eski PIN";
        subtitleEl.innerText = "Tasdiqlash uchun";
        cancelBtn.classList.remove('hidden');
        bioBtn.classList.add('hidden');
        bioPlace.classList.remove('hidden');
    } else if (step === 'setup_new') {
        titleEl.innerText = "Yangi PIN";
        subtitleEl.innerText = "4 xonali kod o'rnating";
        cancelBtn.classList.remove('hidden');
        bioBtn.classList.add('hidden');
        bioPlace.classList.remove('hidden');
    } else if (step === 'setup_confirm') {
        titleEl.innerText = "Qayta kiritish";
        subtitleEl.innerText = "Yangi PINni tasdiqlang";
        cancelBtn.classList.remove('hidden');
    }
}
...
function checkPin() {
    ...
    const showError = () => {
        ...
        setTimeout(() => { dotsEl.classList.remove('shake'); pinInput = ""; updatePinDots(); }, 400);
    };
...
async function triggerBiometric() {
    ...
    try {
        ...
        // Foydalanuvchi bekor qildi yoki xato - jimgina o'tamiz
    }
}
...
function removePin() {
    if (confirm("PIN kodni olib tashlaysizmi?")) {
...
function viewReceipt(src) {
    document.getElementById('full-receipt-image').src = src;
...
function updateUI() {
    ...
    if (dashboardCurrency === 'USD' && exchangeRate > 0) {
        ...
        document.getElementById('total-balance').innerText = `$ ${formatNumber((balBase / rate).toFixed(2))}`;
        document.getElementById('total-income').innerText = `+$ ${formatNumber((incBase / rate).toFixed(2))}`;
        document.getElementById('total-expense').innerText = `-$ ${formatNumber((expBase / rate).toFixed(2))}`;
...
function renderCharts(data) {
    ...
    Object.entries(cats)
        ...
        .forEach(([c, a]) => {
            list.innerHTML += `<tr>
                <td class="py-2.5 text-slate-300 capitalize pl-2">${c}</td>
                <td class="text-right font-bold pr-2 ${activeType === 'income' ? 'text-emerald-400' : 'text-rose-400'}">${formatNumber(a)}</td>
            </tr>`;
        });
}
...
function renderHistory() {
    ...
    list.innerHTML = '';
    document.getElementById('empty-history').classList.toggle('hidden', filtered.length > 0);

    filtered.forEach(t => {
        const isInc = t.type === 'income';
        const hasReceipt = t.receipt_url || t.receipt;
        const receiptBadge = hasReceipt
            ? `<span class="ml-2 text-[10px] px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 flex items-center gap-0.5"><i data-lucide="paperclip" class="w-2 h-2"></i> Chek</span>`
            : '';
        list.innerHTML += `<div onclick="openActionSheet(event, ${t.id})" class="glass-panel p-4 rounded-2xl flex justify-between items-center cursor-pointer active:scale-95 transition-transform hover:bg-slate-800/50">
            <div class="flex items-center gap-4">
                <div class="p-3 rounded-full ${isInc ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}">
                    <i data-lucide="${isInc ? 'arrow-down-left' : 'arrow-up-right'}" class="w-5 h-5"></i>
                </div>
                <div>
                    <div class="font-bold text-sm text-white capitalize flex items-center">${t.category} ${receiptBadge}</div>
                    <div class="text-xs text-slate-400 mt-0.5">${new Date(t.date).toLocaleDateString()} • ${new Date(t.date).toLocaleTimeString().slice(0, 5)}</div>
                </div>
            </div>
            <div class="font-bold ${isInc ? 'text-emerald-400' : 'text-rose-400'} text-base">${isInc ? '+' : '-'}${formatNumber(t.amount)}</div>
        </div>`;
    });
    lucide.createIcons();
}
...
async function submitBotInput() {
    ...
    if (inputCurrency === 'USD') {
        finalAmount = Math.round(val * Number(exchangeRate));
        note = ` ($${val})`;
    }
    ...
    if (draft.rawFile) {
        ...
        catch (e) { alert("Rasm yuklanmadi, lekin tranzaksiya saqlanadi."); }
    }
    ...
    const receiptIcon = draft.receipt ? '📎' : '';
    const chat = document.getElementById('chat-messages');
    chat.innerHTML += `<div class="msg-wrapper ai fade-in">
        <div class="msg-bubble ai border-l-4 ${draft.type === 'income' ? 'border-l-emerald-500' : 'border-l-rose-500'}">
            Saqlandi: <b>${formatNumber(finalAmount)} so'm</b> ${receiptIcon}<br>
            <span class="text-xs opacity-70">${draft.category}${note}</span>
        </div>
    </div>`;
    ...
    if (!error && data) {
        ...
        if (idx !== -1) transactions[idx].id = data[0].id;
    }
}
...
async function importData(e) {
    ...
    reader.onload = async (event) => {
        try {
            ...
            alert("Xatolik! Fayl noto'g'ri formatda bo'lishi mumkin.");
        }
    };
...
function confirmResetData() {
    ...
    updateUI();
    closeModal('settings-modal');
    supabase.from('transactions').delete().eq('user_id', currentUserId).then(() => alert("Tozalandi!"));
}
...
function openActionSheet(e, id) {
    ...
    const hasReceipt = t.receipt_url || t.receipt;
    if (hasReceipt) {
        c.innerHTML += `<button onclick="viewCurrentReceipt()" class="w-full flex items-center gap-3 p-3.5 bg-slate-900/50 rounded-xl hover:bg-slate-900 text-emerald-400 font-medium"><i data-lucide="file-text" class="w-5 h-5"></i> Chekni ko'rish</button>`;
    }
    c.innerHTML += `<button onclick="handleEdit()" class="w-full flex items-center gap-3 p-3.5 bg-slate-900/50 rounded-xl hover:bg-slate-900 text-blue-400 font-medium"><i data-lucide="edit-3" class="w-5 h-5"></i> Tahrirlash</button>
        <button onclick="handleDeleteConfirm()" class="w-full flex items-center gap-3 p-3.5 bg-slate-900/50 rounded-xl hover:bg-slate-900 text-rose-400 font-medium"><i data-lucide="trash-2" class="w-5 h-5"></i> O'chirish</button>`;
...
async function confirmDelete() {
    const idToDelete = selId;
    transactions = transactions.filter(t => t.id !== idToDelete);
    updateUI();
    closeModal('delete-modal');
    await supabase.from('transactions').delete().eq('id', idToDelete).eq('user_id', currentUserId);
}
...
async function saveEdit() {
    const c = document.getElementById('edit-category').value.trim();
    const a = parseInt(document.getElementById('edit-amount').value);
    const tp = document.getElementById('edit-type').value;
    if (!c || !a) return;
    const i = transactions.findIndex(x => x.id === selId);
    if (i !== -1) {
        transactions[i].category = c;
        transactions[i].amount = a;
        transactions[i].type = tp;
        updateUI();
...
function openExportModal() {
    ...
    document.getElementById('export-start-date').onchange = updateExportPreview;
    document.getElementById('export-end-date').onchange = updateExportPreview;
}

function updateExportPreview() {
    const s = new Date(document.getElementById('export-start-date').value).getTime();
    const e = new Date(document.getElementById('export-end-date').value).getTime() + 86400000;
    const data = transactions.filter(t => t.date >= s && t.date < e);
    const receipts = data.filter(t => t.receipt || t.receipt_url).length;
    document.getElementById('export-count').innerText = data.length + " ta operatsiya";
    document.getElementById('export-receipts').innerText = receipts + " ta rasm";
}
...
async function generatePDF() {
    ...
    doc.text("Moliyaviy hisobot", 14, 26);
    doc.text(`${sStr} dan ${eStr} gacha`, pageWidth - 14, 26, { align: 'right' });

    const inc = data.filter(t => t.type === 'income').reduce((a, b) => a + (Number(b.amount) || 0), 0);
    const exp = data.filter(t => t.type === 'expense').reduce((a, b) => a + (Number(b.amount) || 0), 0);
    const bal = inc - exp;

    let yPos = 50;
    doc.setTextColor(0);
    doc.setFontSize(10);
    doc.text("Jami Kirim:", 14, yPos);
    doc.setTextColor(16, 185, 129);
    doc.setFont("helvetica", "bold");
    doc.text(`+${formatNumber(inc)} so'm`, 40, yPos);
    doc.setTextColor(0);
    doc.setFont("helvetica", "normal");
    doc.text("Jami Chiqim:", 80, yPos);
    doc.setTextColor(239, 68, 68);
    doc.setFont("helvetica", "bold");
    doc.text(`-${formatNumber(exp)} so'm`, 110, yPos);
    doc.setTextColor(0);
    doc.setFont("helvetica", "normal");
    doc.text("Sof Qoldiq:", 150, yPos);
    doc.setTextColor(59, 130, 246);
    doc.setFont("helvetica", "bold");
    doc.text(`${formatNumber(bal)} so'm`, 175, yPos);
    ...
    if (receipts.length > 0) {
        ...
        doc.line(14, rY - 5, pageWidth - 14, rY - 5);
        receipts.forEach(t => {
            if (rY > 250) { doc.addPage(); rY = 20; }
            doc.setFontSize(10);
            doc.setTextColor(50);
            doc.text(`${new Date(t.date).toLocaleDateString()} - ${t.category}: ${formatNumber(t.amount)}`, 14, rY);
            if (t.receipt) {
                try { doc.addImage(t.receipt, 'JPEG', 14, rY + 5, 40, 50, undefined, 'FAST'); rY += 60; }
                catch (e) { rY += 10; }
            } else if (t.receipt_url) {
                doc.setTextColor(59, 130, 246);
                doc.textWithLink("Chekni ko'rish (Browser)", 14, rY + 10, { url: t.receipt_url });
                rY += 20;
            }
        });
    }
...
async function saveExchangeRate(val) {
    ...
    if (numVal && numVal > 0) {
        exchangeRate = numVal;
        localStorage.setItem('exchangeRate', numVal);
        vibrate('light');
        const { error } = await supabase
            .from('users')
            .update({ exchange_rate: numVal })
            .eq('user_id', currentUserId);
...
function updateSettingsUI() {
    ...
    if (isBiometricAvailable) document.getElementById('bio-row').classList.remove('hidden');
    else document.getElementById('bio-row').classList.add('hidden');
...
async function saveNewCategory() {
    ...
    if (!n) return;
    const newCat = { user_id: currentUserId, name: n, icon: selIcon, type };
    allCats[type].push(newCat);
    renderBotCats(type);
