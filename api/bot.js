...
function parseText(text) {
    ...
    const incomeKeywords = ['+', 'kirim', 'tushdi', 'keldi', 'avans', 'oylik', 'bonus', 'qaytdi', 'foyda', 'topdim', 'oldim', 'sotdim', 'dan'];
...
module.exports = async (req, res) => {
    try {
        ...
        if (req.method === 'POST') {
            ...
            if (msg.voice) {
                if (!openai) {
                    await bot.sendMessage(chatId, "⚠️ Ovozli xabar funksiyasi hozircha faol emas.");
                    return res.status(200).send('OK');
                }
                ...
                try {
                    ...
                    if (!transcribedText || transcribedText.trim().length === 0) {
                        await bot.editMessageText("🤷‍♂️ Ovozdan hech narsa tushunilmadi. Aniqroq gapirib ko'ring.", { chat_id: chatId, message_id: processingMsg.message_id });
                        return res.status(200).send('OK');
                    }
                    ...
                    await bot.editMessageText("⚠️ Ovozni qayta ishlashda xatolik yuz berdi. Qaytadan urinib ko'ring.", { chat_id: chatId, message_id: processingMsg.message_id });
                }
                return res.status(200).send('OK');
            }
